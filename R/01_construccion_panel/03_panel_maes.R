...
df_tc <- read_excel(here::here("data", "TC_2012-2024.xlsx"), skip = 2) %>%
  rename(Fecha = 1, TC = 2) %>%
  drop_na(Fecha, TC) %>%
  mutate(
    date = ymd(Fecha),
...
df_fob <- read_excel(here::here("data", "harina_pescado_FOB.xlsx"), skip = 2) %>%
...
df_ipc <- read_excel(here::here("data", "IPC_2012_2024.xlsx"), sheet = "Cuadro", skip = 2) %>%
...
df_diesel_raw <- read_excel(here::here("data", "precios_combustibles.xlsx"), sheet = "Petróleo Diesel", skip = 8)
...
df_panel_env <- read_csv(here::here("data", "panel_con_env_puerto.csv"), show_col_types = FALSE)
...
# ==============================================================================
write_csv(df_final, here::here("data", "panel_maestro_definitivo.csv"))
