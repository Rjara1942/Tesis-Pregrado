...
df_panel <- read_csv(here::here("data", "panel_complejo_corregido.csv"), show_col_types = FALSE) %>%
  mutate(
...
df_peru <- read_excel(here::here("data", "BD_PA_2013-2025-sep.xlsx"), skip = 4) %>%
...
df_dolar <- read_excel(here::here("data", "TC_2012-2024.xlsx"), skip = 4, col_names = c("FECHA_STRING", "DOLAR_OBSERVADO")) %>%
  mutate(
...
write_csv(resumen_tests, here::here("outputs", "reportes_intermedios", "resumen_tests_678.csv"))
...
write_csv(coef_final, here::here("outputs", "reportes_intermedios", "coeficientes_finales.csv"))
