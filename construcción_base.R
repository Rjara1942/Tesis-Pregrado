...
harvest_IFOP_jmck_IND_month <- read_excel(" DESEMBARQUES.xlsx", 
...
harvest_IFOP_sardine_IND_month <- read_excel(" DESEMBARQUES.xlsx", 
                             sheet = "INDUSTRIAL (nacional)",
                             range = "N2:W196") %>% 
...
harvest_IFOP_anchoveta_IND_month <- read_excel(" DESEMBARQUES.xlsx", 
           sheet = "INDUSTRIAL (nacional)",
           range = "Y2:AJ273") %>% 
...
harvest_IFOP_jmck_LANCHAS_month <- read_excel(" DESEMBARQUES.xlsx", 
...
harvest_IFOP_sardine_LANCHAS_month <- read_excel(" DESEMBARQUES.xlsx", 
           sheet = "LANCHAS (CentroSur)",
           range = "K2:S170")  %>% 
  mutate(specie = "SARDINA COMUN") %>%
  rename(year = `Años (Fc_Llegada)`) %>%
...
harvest_IFOP_anchoveta_LANCHAS_month <- read_excel(" DESEMBARQUES.xlsx", 
             sheet = "LANCHAS (CentroSur)",
             range = "U2:AC169") %>% 
  mutate(specie = "ANCHOVETA") %>%
  rename(year = `Años (Fc_Llegada)`) %>%
...
harvest_IFOP_jmck_BOTES_month<- read_excel(" DESEMBARQUES.xlsx", 
...
harvest_IFOP_sardine_BOTES_month <- read_excel(" DESEMBARQUES.xlsx", 
             sheet = "BOTES (CentroSur)", 
             range = "K2:S163") %>% 
  mutate(specie = "SARDINA COMUN")  %>%
  rename(year = `Años (Fc_Llegada)`) %>%
...
harvest_IFOP_anchoveta_BOTES_month <- read_excel(" DESEMBARQUES.xlsx", 
             sheet = "BOTES (CentroSur)", 
             range = "U2:AD109") %>% 
  mutate(specie = "ANCHOVETA") %>%
  rename(year = `Años (Fc_Llegada)`) %>%
...
precios_IFOP <- read_excel("2025.04.21.pelagicos_proceso-precios.mp.2012-2024.xlsx", 
              sheet = "PRECIO"
) %>%
...
saveRDS(harvest_price_IFOP_wide,
        file = "/Users/ricardoandejaravalencia/Library/Mobile Documents/com~apple~CloudDocs/tesis_datos/harvest_price_IFOP_wide.rds")
